subroutine golden2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ...
    ! one dimension array
    real(kind=8) :: h,p1,p2,p3,p4,w,itermin
    integer :: i,j,k
	...
	u = uprime
        do iter=1,maxiter
		    ...
		    if (mod(i+j,2)==0) then
			uold(i,j) = u(i,j)
                        u(i,j) = (1.d0-p2)*u(i,j)+p2*0.25d0*(u(i-1,j) + u(i+1,j) + &
	...
	do iter=1,maxiter
		    ...
		    if (mod(i+j,2)==0) then
			uold(i,j) = u(i,j)
                        u(i,j) = (1.d0-p3)*u(i,j)+p3*0.25d0*(u(i-1,j) + u(i+1,j) + &
                                 u(i,j-1) + u(i,j+1) + h**2*f(i,j))
                        dumax = max(dumax, abs(u(i,j)-uold(i,j)))
		    endif
                    enddo
    ...
    do iter=1,maxiter
		...
		if (mod(i+j,2)==0) then
		    uold(i,j) = u(i,j)
                    u(i,j) = (1.d0-p1)*u(i,j)+p1*0.25d0*(u(i-1,j) + u(i+1,j) + &
                                 u(i,j-1) + u(i,j+1) + h**2*f(i,j))
                    dumax = max(dumax, abs(u(i,j)-uold(i,j)))
		endif
                enddo
