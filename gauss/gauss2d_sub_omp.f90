...
subroutine gauss2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ...
    do iter=1,maxiter
        ...
        do j=1,2*n-1
	    if(j>n)then
            	do i = 1,2*n-j 		! lower triangular
		uold(i,i+j-n) = u(i,i+j-n)
                u(i,i+j-n) = 0.25d0*(u(i-1,i+j-n) + u(i+1,i+j-n) + &
                                 u(i,i+j-n-1) + u(i,i+j-n+1) + h**2*f(i,i+j-n))
                dumax = max(dumax, abs(u(i,i+j-n)-uold(i,i+j-n)))
		enddo
	    else
		do i = n-j+1,n			! upper triangular
		uold(i,i+j-n) = u(i,i+j-n)
                u(i,i+j-n) = 0.25d0*(u(i-1,i+j-n) + u(i+1,i+j-n) + &
                                 u(i,i+j-n-1) + u(i,i+j-n+1) + h**2*f(i,i+j-n))
                dumax = max(dumax, abs(u(i,i+j-n)-uold(i,i+j-n)))
